...
dis_flag=0;
use_object_frame_bound=0;
object_frame_bound=[];

keypoints_ref=importdata('small_laptop_auto_keypoint_ref.mat'); % first is frame id, second is object id, third is reinitilisation required (1 for yes)
keypoints=importdata('small_laptop_auto_keypoint.mat');
if(use_object_frame_bound==1)
	object_frame_bound=importdata('small_laptop_bounds.txt');
end
keypoints_ref_pro=[];
numkeypoints=size(keypoints,1);
keypoints_pro=zeros(numkeypoints,3,1);
obs_count=1;
max_num_object=30;
confidence_thres=0.05;
max_frame_not_seen=50;
% mapping = [5,4,6,3,9,8,10,7,1,2]; %from hourglass to mean wireframe
mapping = [1,2,3,4,5,6]; %from hourglass to mean wireframe
	...
	if (keypoints_ref(i,2)>=20 || keypoints_ref(i,2)==0)
		...
		if (use_object_frame_bound==0 || (current_frame >= object_frame_bound(object_id,1) && current_frame <= object_frame_bound(object_id,2)) || (current_frame >= (object_frame_bound(object_id,2)+859)))
			...
			if (last_seen_object(object_id)==-1)
				...
				if (valid_keypoint_count>=numkeypoints-1)
					for z=1:numkeypoints
			...
			elseif ((current_frame - last_seen_object(object_id)) > max_frame_not_seen )
				...
				if (valid_keypoint_count>=numkeypoints-2)
					for z=1:numkeypoints
						keypoints_pro(mapping(z),:,obs_count)=keypoints(z,:,i);
					end				
...
save('small_laptop_auto_keypoint_ref_pro.mat','keypoints_ref_pro');
save('small_laptop_auto_keypoint_pro.mat','keypoints_pro');